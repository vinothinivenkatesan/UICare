	...
	<form style="width:50%;margin:0px;">
		...
		<div style="margin-top:8px;">
			...
			<button type="button" class="btn btn-default"
				style="width: 47%;float: left;" (click)="selectNone(available)"
				[disabled]="!isAnySelected(available)">None</button>
		</div>
	</form>
        
        <div style="width: 10%;margin-left: 15px;padding-top: 20%;">
        
        <button type="button" name="addBtn" class="btn btn-primary"
			style="display:block;width:5%;margin-bottom:8px;float:right;padding-left: 15px; padding-right: 25px;"
			(click)="moveItem(available, confirmed)"
			[disabled]="available.pick.length === 0"><i class="fa fa-forward"></i></button>
                        
                        <button type="button" name="removeBtn" class="btn btn-primary"
			style="display:block;width:3%;margin-left:4px;float:left; padding-left: 15px; padding-right: 25px;"
			(click)="moveItem(confirmed, available)"
			[disabled]="confirmed.pick.length === 0"><i class="fa fa-backward"></i></button>
        </div>

	<form style="width:50%;margin:0 0 0 15px;">		
		<div class="record-picker">
...
export class BsDualList implements DoCheck, OnChanges {
    ...
    ngOnChanges(changeRecord: { [key: string]: SimpleChange }) {
        ...
        if (changeRecord['source']) {
            this.available = new BasicList(BsDualList.AVAILABLE_LIST_NAME);
            this.updatedSource();
            this.updatedDestination();
        }

        if (changeRecord['destination']) {
            this.confirmed = new BasicList(BsDualList.CONFIRMED_LIST_NAME);
            this.updatedDestination();
            this.updatedSource();
        }
    }
